# audio2txt_split
[audio2txt_split](path_to_this) handles transcription of long audio files.
...
1. install some libraries.
    ...
    ```
1. You also need to install [ffmpeg](https://taziku.co.jp/blog/windows-ffmpeg), then add its "bin" directory to PATH, or copy three executable files to "src" directory of audio2txt_split.

## Usage
Run `src/audio2txt_split.py`, then answer the question about the audio file path and options.

After execution, you can find `output/transcribe_{audio_filename}.txt`.
