...
def make_or_clear_dir(dir_in):
    if not os.path.exists(dir_in):
        ...
        # tmpdir = os.path.join(audio_split_dir, 'tmp')
        # if not os.path.exists(tmpdir):
        #     os.makedirs(tmpdir)
        # for file_name in os.listdir(audio_split_dir):
        #     file_path = os.path.join(audio_split_dir, file_name)
        #     if os.path.isfile(file_path):
        #         os.rename(file_path, os.path.join(tmpdir, file_name))
...
if __name__ == "__main__":
    ...
    while 1:
        model_size = input("Model size (tiny, base, small, medium, large, turbo) (Enter->small):")
        if model_size == "":
            model_size = "small"
    ...
    # print(f"Audio filename (without extension): {audio_filename}")

    outputDirPath = os.path.join("..", "output")
    audio_split_dir = os.path.join("..", "tmp")
    log_path = os.path.join(outputDirPath, f"log_{audio_filename}.txt")
    transcribe_res_path = os.path.join(outputDirPath, f"transcribe_{audio_filename}.txt")
    ### ----------------------------------
    ...
    for audio_path in out['output_file_list']:
        print(f"# transcribing {audio_path}...", end="")
